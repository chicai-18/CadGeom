 ...
 *
 * @note 这个文件和 examples/qt_viewer/HostCommands.h 逐字相同，SampleScene 也是。
 *       那不是疏忽：它们只跟引擎打交道，一个 MFC 或 Qt 的符号都没有 —— 「宿主侧
 *       的引擎代码」和「宿主用什么 UI 框架」本来就是两件事，换框架时这两个文件
 *       原样搬过来就行。
 */
#ifndef CADGEOM_MFC_VIEWER_HOSTCOMMANDS_H
#define CADGEOM_MFC_VIEWER_HOSTCOMMANDS_H
...
#endif // CADGEOM_MFC_VIEWER_HOSTCOMMANDS_H

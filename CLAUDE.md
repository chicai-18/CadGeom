...
Milestone **M0 complete** (skeleton). See `docs/architecture.md` §9 for the plan.
...
| Geometry kernel, tessellation | **M2** — creation calls return `NotImplemented` |
| Vulkan renderer, viewports, camera | **M1** — `CreateViewport` returns null |
| BVH, picking, gizmos | **M3** |
...
Vulkan-free for the same reason.
...
The Vulkan SDK is the one dependency not vendored. Without it the build drops
the renderer and says so; everything through M0 still builds and runs. M1 needs
LunarG >= 1.3.275 with `VULKAN_SDK` set and `glslc` on hand — note that a
driver-provided `vulkan-1.dll` is *not* the SDK.
